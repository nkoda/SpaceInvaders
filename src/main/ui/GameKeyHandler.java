...
public class GameKeyHandler {
    ...
    public void keyPressed(int keyCode) {
        ...
        } else if (keyCode == KeyEvent.VK_Z) { game.getSaveGame().saveGame(); }
        else if (keyCode == KeyEvent.VK_V) {
            game.getGameScene().dispatchEvent(new WindowEvent(game.getGameScene(), WindowEvent.WINDOW_CLOSING));
        } else { playerControl(keyCode); }
    }

    // modifies: this
    // effects: turns tank in response to key code
    public void playerControl(int keyCode) {
        if (keyCode == KeyEvent.VK_A) { game.getUserPlayer().moveLeft(); }
        else if (keyCode == KeyEvent.VK_D) { game.getUserPlayer().moveRight(); }
        else if (keyCode == KeyEvent.VK_W) { game.getUserPlayer().moveUp(); }
        else if (keyCode == KeyEvent.VK_S) { game.getUserPlayer().moveDown(); }
    }
}